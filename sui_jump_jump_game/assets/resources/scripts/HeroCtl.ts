import { _decorator, Component, Node, Vec3, tween, Label, Sprite, Color, Prefab, instantiate, UITransform, resources, SpriteFrame } from 'cc';
...
export class HeroCtl extends Component {
     ...
     */
    init_hero(parent: Node, pos: Vec3, jumperName?: string) {
        this.node.setParent(parent);
        this.node.setPosition(pos.x, pos.y + 200);
        const roleNode = this.node.getChildByName("body").getChildByName("role")
        resources.load(`images/${jumperName ?? "defaultJumper"}/spriteFrame`, SpriteFrame, (err, frame) => {
            roleNode.getComponent(Sprite).spriteFrame = frame;
            //弹跳动画
            tween(this.node)
