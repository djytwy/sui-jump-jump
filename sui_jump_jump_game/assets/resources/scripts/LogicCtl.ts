import { _decorator, Component, Node, Prefab, Vec3, instantiate, Input, input, EventTouch, UITransform, tween } from 'cc';
...
export class LogicCtl extends Component {
    ...
    // sui 上的票对象
    ticketObj: SuiObjectResponse = null;
    ticketLevel: string = "";
    ...
    start() {
        ...
        this.run_game(-1);
        EventDispatcher.get_instance().target.on(EventDispatcher.RELOAD_GAME_SKIN, (status: number, jumperName: string) => {
            this.run_game(status, jumperName);
        }, this);
    }
    ...
    touch_end(e: EventTouch) {
        ...
        this.hero_ctl.jump_by_box(this.box_arr[this.box_arr.length - 1],
            this.touch_time, async (score: number, collider_node: Node) => {
                //判断是否发生碰撞
     ...
     */
    run_game(game_state: number, jumperName?: string) {
        //状态设置
        ...
        this.hero_ctl.init_hero(this.node, box_1.get_jump_position(), jumperName ?? window.localStorage.getItem("skin"));
        //屏幕移动一下,根据箱子和参考点
        this.move();
    }
    ...
    show_over_page() {
        ...
        //     return
        // } else if (!gameManager.instance.zkGameInfo && !gameManager.instance.walletInfo) {
        //     //发送游戏结束事件,用以打开over_page面板 pvp
        //     EventDispatcher.get_instance().target.emit(EventDispatcher.SHOW_OVER_WINDOW_FREE);
        //     GameData.set_game_state(4);
        //     return
        // } else if ((gameManager.instance.zkGameInfo && gameManager.instance.zkGameInfo.address) || gameManager.instance.walletInfo.address) {
        //     ticket = window.localStorage.getItem("ticketObj")
        // }
        const mode = gameManager.instance.gameMode
        if (mode === "pvp") {
            //发送游戏结束事件,用以打开over_page面板 pvp
