...
const EAmountMustBeEqualWithGoldTicketPrice: u64 = 2;
const EAmountMustBeEqualWithSliverTicketPrice: u64 = 3;
const EAmountMustBeEqualWithbronzeTicketPrice: u64 = 4;
...
entry fun buyTicket (money: Coin<SUI>, ticketPrice: &TicketPrice, pool: &mut PrizePool,clock: &Clock,ctx: &mut TxContext) {
    ...
    if (poolLevel == utf8(b"gold")) {
        assert!(money.value() == ticketPrice.goldPrice, EAmountMustBeEqualWithGoldTicketPrice);
        let ticket = Ticket {
            id: object::new(ctx),
            price: money.value(),
        ...
        assert!(money.value() == ticketPrice.sliverPrice, EAmountMustBeEqualWithSliverTicketPrice);
        let ticket = Ticket {
            id: object::new(ctx),
            price: money.value(),
            buyTime: timestamp_ms(clock),
        ...
        assert!(money.value() == ticketPrice.bronzePrice, EAmountMustBeEqualWithbronzeTicketPrice);
        let ticket = Ticket {
            id: object::new(ctx),
            price: money.value(),
            buyTime: timestamp_ms(clock),
